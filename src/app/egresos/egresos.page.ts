  ...
  export class EgresosPage implements OnInit, OnDestroy {
    ...
    usuarios:         Usuario[] = [];
    cargando          = false;
    ingresosDelDia    = 0;   // efectivo + transf + cheques + abonos del cierre activo
    ...
    /** Cuánto queda disponible para egresos hoy */
    ...
    // ---- CARGA COMBINADA (egresos + ventas del día) -------------------------
    cargarDatos() {
      this.cargando = true;
      this.egresos  = [];
      // Primero obtenemos el cierre activo para saber su id e ingresos reales
      this.http.get<any>(`${this.API}/cierres/activo`, { headers: this.getHeaders() }).subscribe({
        next: (cierre) => {
          ...
          // Luego cargamos egresos del cierre activo
    ...
    // Alias para que el template pueda llamar cargarEgresos igual que antes
    ...
    private calcularIngresos(ventas: any[]): number {
      return ventas.reduce((acc, v) => {
        ...
        // Solo contamos efectivo, transferencia y cheques (NO crédito/pendiente)
        if (['efectivo', 'transferencia', 'cheques'].includes(tipo)) return acc + total;
        return acc;
      }, 0);
    }
    ...
    guardarEdicion() {
      ...
      // ── VALIDACIÓN SALDO (al editar: el saldo disponible + el valor original del egreso) ──
      if (valido) {
